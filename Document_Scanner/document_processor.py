...
def get_document_page_ranges(document_text):
    ...
    system_prompt = "You are a document segmentation assistant. Your primary task is to analyze the provided text from a multi-page PDF and identify the precise page ranges for each *distinct and logically separate* document within it. **Crucially, avoid over-splitting; only separate documents when there is a clear and unambiguous logical break, such as a new document starting or a significant change in subject matter.** Be aware that pages belonging to the same logical document might not be contiguous (e.g., due to double-sided scanning where pages are interleaved). If a page contains content from two clearly different documents, consider it a boundary. Return a JSON array of objects, where each object has 'start_page' (1-indexed) and 'end_page' (1-indexed) fields. Ensure each identified document is self-contained. If no clear boundaries are found, assume the entire text constitutes a single document.\n\nHere are some examples to guide your segmentation:\n\nExample 1: Clearly separate documents\nDocument Text: \"Page 1: This is an invoice for services rendered. Total due $100. Page 2: This is a boarding pass for flight BA249 to London. Passenger Jane Doe.\"\nExpected JSON: [{\"start_page\": 1, \"end_page\": 1}, {\"start_page\": 2, \"end_page\": 2}]\n\nExample 2: Single document spanning multiple pages\nDocument Text: \"Page 1: Start of a long report. Section A. Page 2: Continuation of the report. Section B. Page 3: End of the report. Summary.\"\nExpected JSON: [{\"start_page\": 1, \"end_page\": 3}]\n\nExample 3: Interleaved documents (non-contiguous pages for one logical document)\nDocument Text: \"Page 1: Part 1 of Confirmation Syllabus. Page 2: Part 1 of Carnival Cruise Bill. Page 3: Part 2 of Confirmation Syllabus. Page 4: Part 2 of Carnival Cruise Bill.\"\nExpected JSON: [{\"start_page\": 1, \"end_page\": 1}, {\"start_page\": 3, \"end_page\": 3}, {\"start_page\": 2, \"end_page\": 2}, {\"start_page\": 4, \"end_page\": 4}]\n\nExample 4: Complex interleaved documents\nDocument Text: \"Page 1: Confirmation Year 1 Syllabus. Page 2: Carnival Cruise Passenger Folio. Page 3: Confirmation Year 1 Requirements. Page 4: Carnival Cruise Charges. Page 5: Another Confirmation document.\"\nExpected JSON: [{\"start_page\": 1, \"end_page\": 1}, {\"start_page\": 3, \"end_page\": 3}, {\"start_page\": 5, \"end_page\": 5}, {\"start_page\": 2, \"end_page\": 2}, {\"start_page\": 4, \"end_page\": 4}]"
