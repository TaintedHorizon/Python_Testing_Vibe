...
def get_single_documents_for_batch(batch_id):
    ...
    These are the per-file records generated by the improved single document
    pipeline (single_documents table). We expose only the columns required by
    the manipulation UI so the route layer does not embed raw SQL.

    Args:
        batch_id (int): Batch primary key.

    Returns:
        list[sqlite3.Row]: Rows ordered by id for deterministic pagination. Each
            row includes: id, original_filename, original_pdf_path,
            ai_suggested_category, ai_suggested_filename, ai_confidence,
            ai_summary, ocr_text, ocr_confidence_avg.
    """
    conn = get_db_connection()
    try:
        rows = conn.execute(
