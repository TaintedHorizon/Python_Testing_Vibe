...
def serve_single_pdf(doc_id: int):
    ...
    Test expectations (rotation_serving & grouped_rotation):
      ...
      * Touching timestamp without changing angle => identical bytes (reuse)
        ...
        Implementation:
            ...
            * Cache location: test-scoped tempdir (TEST_TMPDIR/TMPDIR or system temp) with name rot_cache_doc_<id>_<angle>.pdf
    ...
    try:
            ...
            if not is_test_mode:
                ...
                if not any(pdf_abs.startswith(d + os.sep) or pdf_abs == d for d in allowed_abs):
                    if os.path.exists(pdf_abs):
                        ...
                        # Also allow test-scoped temp dirs (TEST_TMPDIR/TMPDIR) which tests may use for rotation cache
                        try:
                            import tempfile as _tempfile
                            tmp_root = os.path.abspath(os.getenv('TEST_TMPDIR') or os.getenv('TMPDIR') or _tempfile.gettempdir())
                        except Exception:
                            # Fallback to configured env TMPDIR or current working directory if tempfile import fails
                            tmp_root = os.path.abspath(os.getenv('TEST_TMPDIR') or os.getenv('TMPDIR') or os.getcwd())
                        if pdf_abs.startswith(tmp_root + os.sep) or pdf_abs == tmp_root:
                            logger.warning(f"TEST_MODE: allowing tempdir PDF path: {pdf_abs}")
                        else:
                            logger.warning(f"Blocked PDF serve outside allowed dirs: {pdf_abs}")
                            return jsonify(create_error_response("Access denied", 403)), 403
        except Exception:
        ...
        try:
            ...
            tmpdir = os.getenv('TEST_TMPDIR') or os.getenv('TMPDIR')
            if not tmpdir:
                import tempfile as _temp
                tmpdir = _temp.gettempdir()
        except Exception:
            tmpdir = os.getenv('TEST_TMPDIR') or os.getenv('TMPDIR') or os.getcwd()
        cache_path = os.path.join(tmpdir, cache_basename)
        ...
        if regenerate:
            try:
                ...
                # Ensure tmpdir exists (respect TEST_TMPDIR/TMPDIR/system tempdir selection)
                try:
                    os.makedirs(os.path.dirname(cache_path), exist_ok=True)
                except Exception:
                    # If we cannot create the dir, continue and let save() attempt to write
                    pass
