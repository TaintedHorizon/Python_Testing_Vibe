...
def poll_smart_processing_status(token: str, base_url: str = "http://127.0.0.1:5000",
    ...
    return (last if last is not None else None), {"polls": polls, "stalled": stall_count >= stall_limit, "completed": False, "last_progress": last_progress}
"""Helper utilities for polling smart processing fallback status in tests.
...
Placed in `doc_processor/tests/e2e/helpers` so it can be imported by tests
as `doc_processor.tests.e2e.helpers.smart_status_helper` and by scripts
via a local import path.
"""
import time
import requests
from typing import Any, Dict, Optional, Tuple
