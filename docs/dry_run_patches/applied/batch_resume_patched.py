"""
Dry-run wrapper for `doc_processor.dev_tools.batch_resume`.

Ensures WIP and processed directories are test-scoped and delegates to original.
"""
import os
import tempfile
...
os.environ.setdefault("WIP_DIR", os.path.join(base, "wip"))
...
def _main() -> int:
    ...
    mod = import_module("doc_processor.dev_tools.batch_resume")
    if hasattr(mod, "main"):
        return mod.main()
    if hasattr(mod, "run"):
        return mod.run()
...
Sets WIP/processed/backup paths to test-scoped locations (prefer `TEST_TMPDIR`)
before delegating to the original module.
"""
...
Ensures any temp or backup paths are redirected to TEST_TMPDIR for tests.
"""
import os
...
ensure_env_var_from_test("BATCH_RESUME_BACKUP_DIR", "batch_resume_backups")
