"""
Dry-run wrapper for `doc_processor.dev_tools.force_reset_batch`.

Sets WIP backup base to a safe test location then delegates.
"""
import os
...
ensure_env_var_from_test("WIP_BACKUP_BASE", "wip_backups")


def _main() -> int:
    from importlib import import_module

    mod = import_module("doc_processor.dev_tools.force_reset_batch")
    if hasattr(mod, "main"):
        return mod.main()
    if hasattr(mod, "run"):
        return mod.run()
