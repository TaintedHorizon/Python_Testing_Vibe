...
This wrapper redirects outputs to TEST_TMPDIR when running under tests and delegates
to the original module. Keeps behavior non-invasive by using os.environ.setdefault.
"""
import os
import tempfile
...
def _main() -> int:
    ...
    mod = import_module("archive.legacy.Document_Scanner_Gemini_outdated.document_processor")
    if hasattr(mod, "main"):
        return mod.main()
    if hasattr(mod, "run"):
        return mod.run()
