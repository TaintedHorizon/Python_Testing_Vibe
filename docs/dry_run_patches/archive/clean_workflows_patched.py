"""
Dry-run wrapper for `tools/clean_workflows.py`.

Routes any file operations into a TEST_TMPDIR-scoped workspace and delegates.
"""
import os
import tempfile
...
os.environ.setdefault("CLEAN_WORKFLOWS_TMP", os.path.join(base, "clean_workflows"))


def _main() -> int:
    from importlib import import_module

    mod = import_module("tools.clean_workflows")
    if hasattr(mod, "main"):
        return mod.main()
    if hasattr(mod, "run"):
        return mod.run()
