"""
Dry-run wrapper for `doc_processor.dev_tools.diagnose_grouping_block`.

Ensures any temporary outputs go under TEST_TMPDIR.
"""
import os
...
ensure_env_var_from_test("DIAGNOSE_OUTPUT_DIR", "diagnose_outputs")


def _main() -> int:
    from importlib import import_module

    mod = import_module("doc_processor.dev_tools.diagnose_grouping_block")
    if hasattr(mod, "main"):
        return mod.main()
    if hasattr(mod, "run"):
        return mod.run()
