"""
Dry-run wrapper for `tools.gamelist_editor.gamelist_xml_editor`.

Routes any file writes to TEST_TMPDIR and delegates to the original module.
"""
import os
import tempfile
...
def _main() -> int:
    ...
    mod = import_module("tools.gamelist_editor.gamelist_xml_editor")
    if hasattr(mod, "main"):
        return mod.main()
    if hasattr(mod, "run"):
        return mod.run()
